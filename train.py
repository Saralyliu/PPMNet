...
from model import OurPPMNet as PI_CLIP
from util import dataset1 as dataset
from util import transform1 as transform
from util import transform_tri1 as transform_tri
from util import config,transform_cam
...
def get_parser():
    ...
    #                     help='config file')
    parser.add_argument('--config', type=str, default='config/pascal/pascal_split3_resnet50_manet.yaml',
                        help='config file')
...
def main():
    ...
    train_transform_cam = transform_cam.Compose([
        transform_cam.RandScale([args.scale_min, args.scale_max]),
        transform_cam.RandRotate([args.rotate_min, args.rotate_max], padding=mean, ignore_label=args.padding_label),
        transform_cam.RandomHorizontalFlip(),
        transform_cam.Resize([args.train_h, args.train_w]),
        transform_cam.ToTensor()])
    ...
    if args.data_set == 'pascal' or args.data_set == 'coco':
        train_data = dataset.SemData(split=args.split, shot=args.shot, data_root=args.data_root,
                                     ...
                                     transform=train_transform, transform_tri=train_transform_tri,transform_cam=train_transform_cam, mode='train', \
    ...
    if args.evaluate:
        if args.resized_val:
            ...
            val_transform_cam = transform_cam.Compose([
                transform_cam.Resize(size=args.val_size),
                transform_cam.ToTensor()])
        ...
        if args.data_set == 'pascal' or args.data_set == 'coco':
            val_data = dataset.SemData(split=args.split, shot=args.shot, data_root=args.data_root,
                                       ...
                                       transform=val_transform, transform_tri=val_transform_tri,transform_cam=val_transform_cam, mode='val', \
                                       data_set=args.data_set, use_split_coco=args.use_split_coco)
...
def train(train_loader, val_loader, model, optimizer, epoch):
    ...
    for i, (input, q_cam,s_cam, target, target_b, s_input, s_mask, subcls, class_name) in enumerate(train_loader):
        ...
        q_cam = q_cam.cuda(non_blocking=True)
        s_cam = s_cam.cuda(non_blocking=True)
        target = target.cuda(non_blocking=True)
        target_b = target_b.cuda(non_blocking=True)

        output, main_loss, aux_loss1, aux_loss2 = model(s_x=s_input, q_cam=q_cam, s_cam=s_cam, s_y=s_mask, x=input,
        ...
        # -----------------------  SubEpoch VAL  -----------------------
        if args.evaluate and args.SubEpoch_val and (args.epochs <= 30 and epoch % 1 == 0 and epoch > 0) and (
                i == round(len(train_loader) / 2)):  # <if> max_epoch<=100 <do> half_epoch Val  # <if> max_epoch<=100 <do> half_epoch Val
            loss_val, FBIoU, FBIoU_m, mIoU, mIoU_m, mIoU_b, pIoU = validate(val_loader, model)
            val_num += 1
            # save model for <testing>
            if mIoU > best_miou:
...
def validate(val_loader, model):
    ...
    for e in range(db_epoch):
        for i, (input, q_cam, s_cam, target, target_b, s_input, s_mask, subcls, class_name, ori_label,
                ori_label_b) in enumerate(val_loader):
            ...
            output, meta_out, base_out = model(s_x=s_input, q_cam=q_cam, s_cam=s_cam, s_y=s_mask, x=input,
                                               y_m=target, y_b=target_b,
                                               cat_idx=subcls, class_name=class_name)
