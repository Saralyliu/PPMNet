...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/pascal/pascal_split0_resnet50_manet.yaml', help='config file') #config/coco/coco_split0_resnet50_manet.yaml
...
def get_model(args):
    ...
    model = model.cuda()
    # data = [(1, 3, 473, 473), (1, 473, 473), (1, 1, 473, 473), (1, 473, 473), (1, 473, 473), (1, 1, 3, 473, 473),[1]]
    # summary(model, data)
    # Resume
    get_save_path(args)
...
def validate(val_loader, model, val_seed):
    ...
    for e in range(db_epoch):
        for i, (input, q_cam,s_cam, target, target_b, s_input, s_mask, subcls, class_name, ori_label, ori_label_b) in enumerate(val_loader):
            ...
            start_time = time.time()
            # flops, params = profile(model, inputs=(input, q_cam,s_cam, class_name, target, target_b, s_input, s_mask, subcls))
            # 输出结果
