...
class Compose(object):
    ...
    def __call__(self, image, label, label2,cam):
        for t in self.segtransform:
            image, label, label2, cam = t(image, label, label2,cam)
        return image, label, label2, cam
...
class Resize(object):
    ...
    def __call__(self, image, label, label2, cam):
        ...
        cam = cv2.resize(cam.astype(np.float32), dsize=self.size, interpolation=cv2.INTER_NEAREST)
        return image, label, label2, cam
    ...
    def __call__(self, image, label, label2, cam):
        if random.random() < self.p:
            ...
            label2 = cv2.warpAffine(label2, matrix, (w, h), flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT,
                                    borderValue=self.ignore_label)
            cam= cv2.warpAffine(cam, matrix, (w, h), flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT,
                                    borderValue=0)
        return image, label, label2, cam
