...
class SemData(Dataset):
    def __init__(self, split=3, shot=1, data_root=None, base_data_root=None, data_list=None, data_set=None,
                 ...
                 transform=None, transform_tri=None,transform_cam=None, mode='train', ann_type='mask', \
        ...
        self.posude_root = os.path.join(self.data_root, mode)
        ...
        self.transform_cam = transform_cam
    ...
    def __getitem__(self, index):
        ...
        query_name = tmp_sperate[2]
        # if 'pascal' in image_path:
        #     query_name = tmp_sperate[5]
        # else:
        #     query_name = tmp_sperate[4]
        image = cv2.imread(image_path, cv2.IMREAD_COLOR)
        qori_height, qori_width = image.shape[:2]
        ...
        qscam_root = os.path.join(self.posude_root, str(class_chosen-1),str(query_name.split('.')[0]+'.npy'))
        q_cam=np.load(qscam_root)
        if q_cam.shape!=image.shape[:2]:
            q_cam = scale_cam_image([q_cam], (qori_width, qori_height))[0]
        ...
        for k in range(self.shot):
            ...
            sori_height, sori_width = support_image.shape[:2]
            support_image = cv2.cvtColor(support_image, cv2.COLOR_BGR2RGB)
            support_image = np.float32(support_image)

            tmp_sperate = support_image_path.split('/')
            s_name = tmp_sperate[2]
            sscam_root = os.path.join(self.posude_root, str(class_chosen - 1), str(s_name.split('.')[0] + '.npy'))
            s_cam = np.load(sscam_root)
            if s_cam.shape != support_image.shape[:2]:
                s_cam = scale_cam_image([s_cam], (sori_width, sori_height))[0]
            scam_list_ori.append(s_cam)
        ...
        scam_list = [[] for _ in range(self.shot)]

        if self.transform is not None:
            image, label, label_b, q_cam = self.transform_tri(image, label, label_b, q_cam)  # transform the triple
            # q_cam = self.transform_cam(q_cam)
            for k in range(self.shot):
                support_image_list[k], support_label_list[k],scam_list[k] = self.transform(support_image_list_ori[k], support_label_list_ori[k],scam_list_ori[k])
                # scam_list[k]=self.transform_cam(scam_list_ori[k])
        ...
        s_cam = scam_list[0].unsqueeze(0)
        for i in range(1, self.shot):
            s_cam = torch.cat([scam_list[i].unsqueeze(0), s_cam], 0)

        # Return
        if self.mode == 'train':
            return image, q_cam,s_cam, label, label_b, s_x, s_y, subcls_list, class_chosen-1
        elif self.mode == 'val':
            return image, q_cam,s_cam, label, label_b, s_x, s_y, subcls_list, class_chosen-1, raw_label, raw_label_b
        elif self.mode == 'demo':
